...
// Here I have added all that change the values/experience of the video.
// For my extra feature, I decided to add a dark mode toggle, as I believe everyone should have the option 
// on how the website is presented to them depending on their current viewing situation.
function darkModeToggle() {
  current =  document.getElementById("darkButton");
  var element = document.body;
  element.classList.toggle("darkmode");
  if (current.innerHTML === "Turn the Lights Off") {
      current.innerHTML = "Turn the Lights On";
  } else {
    current.innerHTML="Turn the Lights Off";
  }
}

// As for the rewind and fast forward buttons, I decided to allow the user to change the video time 
// increments by 5, as this is usually the c ase with most media players, and has become familiar and conventional for most.
function rewindSpeed(){
...
// As for the loop button, and I gave the user a choice to restart the video when paused or once it is over. 
// This allows them to replay the video without have to scrub back to the start or spam the rewind button.
function loopVideo() {
...
// Simple mute sound button, as every loves the ability to mute audio. I decided not to add an autoplay feature as it can be quiet intrusive, but 
// there should always be the option to mute audio.
function muteSound() {
...
// For the fullscreen button, I used a simple fullscreen conversion that pulls up default media controls, as they already have a simple built in design for a fullscreen player.
function fullscreen() {
...
darkButton.addEventListener('click', darkModeToggle)
playButton.addEventListener('click', playPause);
rewindButton.addEventListener('click', rewindSpeed);
forwardButton.addEventListener('click', forwardSpeed);
